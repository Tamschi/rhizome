            ...
            if lookahead.peek(Token![ref]) {
                ...
                initializer_body.push({
                    ...
                    let map_missing_dependency = quote_spanned! {question.span=>
                        .map_err(|error| #rhizome::error::auto_provision_error::AutoProvisionError::MissingDependency(
...
impl ExtractableDeclaration {
    pub fn into_tokens(self) -> Result<TokenStream> {
        ...
        let default_provide = if abstract_token.is_some() {
            ...
            Some(quote! {
                pub fn provide(node: &mut #rhizome::Node) -> Result<(), #rhizome::error::auto_provision_error::AutoProvisionError> {
                    let dyn_arc: std::sync::Arc<Self> = std::sync::Arc::new(#name_impl::new(node)?);
                    node.provide(
                        <#name_impl as #rhizome::TypeKey<#rhizome::error::auto_provision_error::AutoProvisionError>>::key(),
                        Box::new(dyn_arc)
                    ).map_err(|_| #rhizome::error::auto_provision_error::AutoProvisionError::KeyExists)
                }
            })
        };
        ...
        let default_impl = if abstract_token.is_some() {
            ...
            Some(quote! {
                impl #name_impl {
                    pub fn new(node: &#rhizome::Node) -> Result<Self, #rhizome::error::auto_provision_error::AutoProvisionError> {
                        Ok(Self {
                            #(#initializer_body)*
                        })
                    }

                    fn factory(node: &#rhizome::Node) -> Result<Box<dyn core::any::Any>, #rhizome::error::auto_provision_error::AutoProvisionError> {
        ...
        Ok(quote! {
            ...
            #default_impl
            impl #rhizome::TypeKey<#rhizome::error::auto_provision_error::AutoProvisionError> for #name_impl {
                fn provision() -> #rhizome::Provision<#rhizome::error::auto_provision_error::AutoProvisionError, Box<dyn core::any::Any>> {
                    #provision
                }
            }
        })
